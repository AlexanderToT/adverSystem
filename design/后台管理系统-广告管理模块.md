...
-   **`AdvertisementListPage.tsx`**: 展示广告列表，提供搜索、筛选（如按状态、时间、类型等）、排序、添加新广告等功能。
-   **`AdvertisementFormPage.tsx`** (或 `AdvertisementFormModal.tsx`): 用于创建或编辑广告，分步骤或在一个表单中收集广告信息。
...
-   **`AdForm.tsx`**: 复杂表单，使用 `Form`, `Input`, `Select`, `TextArea`, `DatePicker`, `Upload`, `Switch` 等组件。
-   **`MaterialUploader.tsx`**: 广告素材上传组件，直接将文件上传到R2，并返回URL和其他元数据，以JSON格式存储。
-   **`StatsChart.tsx`**: 使用图表库（如 Ant Design Charts）展示广告效果数据。
...
src/
├── controllers/
│   └── advertisement.controller.ts # 请求处理
├── routes/
│   └── advertisement.routes.ts # 相关路由
├── service/
│   └── advertisement.service.ts # 核心业务逻辑
├── validation/ 
│   └── advertisement.validation.ts #  输入验证 (Zod)         
├── middleware/
