...
// 登录服务
export const loginUser = async (loginInput, c) => {
...
// 登出服务
export const logoutUser = async (token, jwtBlacklist) => {
    // 解析JWT获取过期时间，计算剩余有效期
    try {
...
// 获取当前用户信息
export const getCurrentUser = async (userId, c) => {
    const db = getDb(c.env);
    const user = await db.query.users.findFirst({
...
// 用户列表查询（分页）
export const getUsers = async (page, limit, c, search, role) => {
    const db = getDb(c.env);
    // 构建查询条件
    let users;
    if (search) {
