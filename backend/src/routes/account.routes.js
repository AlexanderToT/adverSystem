...
// 创建用户 (仅超级管理员)
accountRouter.post('/users', authorize(['super_admin']), zValidator('json', createUserSchema), accountController.createUserHandler);
// 获取单个用户
accountRouter.get('/users/:id', accountController.getUserByIdHandler);
// 更新用户 (仅超级管理员或用户自己)
accountRouter.put('/users/:id', zValidator('json', updateUserSchema), accountController.updateUserHandler);
// 删除用户 (仅超级管理员)
accountRouter.delete('/users/:id', authorize(['super_admin']), accountController.deleteUserHandler);
// 修改密码 (仅超级管理员或用户自己)
accountRouter.put('/users/:id/change-password', zValidator('json', changePasswordSchema), accountController.changeUserPasswordHandler);
